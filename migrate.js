...
content = content.replace(
  ...
  "role: 'SYS_ADMIN' | 'SCHOOL_ADMIN' | 'TREASURER' | 'KORLAS' | 'STUDENT' | 'PARENT';\n  schoolId?: string;"
);
content = content.replace(
...
content = content.replace(/role: 'ADMIN'/g, "role: 'SCHOOL_ADMIN'");
...
content = content.replace(
  /id: `tx-excess-\$\{Date\.now\(\)\}`,/g,
  "id: `tx-excess-${Date.now()}`,\n        schoolId: activeSchoolId,"
);
content = content.replace(
...
content = content.replace(/"\{activeSchool\.name\}"/g, 'activeSchool.name');
content = content.replace(/`\{activeSchool\.name\}`/g, 'activeSchool.name');
content = content.replace(/sdn08pagi\.sch\.id/g, "${activeSchool.domain}");
content = content.replace(/alt="\{activeSchool\.name\} Showcase"/g, 'alt={`${activeSchool.name} Showcase`}');
